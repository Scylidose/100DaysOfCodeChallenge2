# 100 Days of Code Challenge

----------------------------------------------
        100 Days of Code Challenge | ROUND 2
----------------------------------------------

----------------------------------------------
     Week 11 Recap - 

----------------------------------------------

## Day 75: 27 July 2019

- Done Section 7 of the TensorFlow 2.0: A Complete Guide on the Brand New Tensorflow course

- Started to read Pattern Recognition and Machine Learning book

**Aprox Time:** ***3 hours***

## Day 74: 26 July 2019

- Started Section 7 of the TensorFlow 2.0: A Complete Guide on the Brand New Tensorflow course

- Read articles about Reinforcement Learning on Medium website

**Aprox Time:** ***2 hours***

## Day 73: 25 July 2019

- Done Section 6 of the TensorFlow 2.0: A Complete Guide on the Brand New Tensorflow

- Read articles explaining how Transfer Learning works on TowardsDataScience

Here's what I got on my TL model that predicts if it's a picture of a cat or a dog.

![tl1](/Calendar/img/tl1.png)
![tl2](/Calendar/img/tl2.png)

**Aprox Time:** ***4 hours***

## Was absent a few days to concentrate on my final exams

## Day 72: 19 July 2019

- Read Object Detection using Deep Learning Approaches by Rahul A. on TowardsDataScience

- Read A Step-by-Step Introduction to the Basic Object Detection Algorithms (Part 1) by Pulkit S. on Analytics Vidhya

**Aprox Time:** ***2 hours***

## Day 71: 18 July 2019

- Done Section 5 from the Udemy course : TensorFlow 2.0: A Complete Guide on the Brand New Tensorflow

- Read Why you need to start using Embedding Layers article by Rutger R. and Understanding RMSprop article by Vitaly B. on TowardsDataScience

**Aprox Time:** ***2 hours***

----------------------------------------------
     Week 10 Recap - Neural Networks

- Continued the Udemy course : TensorFlow 2.0: A Complete Guide on the Brand New Tensorflow

- Done an ANN and CNN

- Read articles that talks about ANN, CNN and RNN

----------------------------------------------

## Day 70: 17 July 2019

- Done Section 17 (RNN Theory) from the Udemy course : TensorFlow 2.0: A Complete Guide on the Brand New Tensorflow

**Aprox Time:** ***1.5 hour***

## Day 69: 16 July 2019

- Read Intro to CNN by Jianxin Wu.

- Done some research about LSTM Networks

- Read articles about RNN on Medium website

- Read Deep Visual-Semantic Alignments for Generating Image Descriptions paper by Andrej Karpathy & Li Fei-Fei

**Aprox Time:** ***4 hours***

## Day 68: 15 July 2019

- Started to read Intro to CNN by Jianxin Wu.

- Done Section 4 from the Udemy course : TensorFlow 2.0: A Complete Guide on the Brand New Tensorflow.

So I built my first CNN and I tried it with a photo of my dog, and it worked with a high accuracy😆

![cnn1](/Calendar/img/cnn1.png)

**Aprox Time:** ***3 hours***

## Day 67: 14 July 2019

- Followed A Neural Network in 11 lines of Python (Part 1) article by iamtrask

- Done Section 15 (AAN Theory)  and Section 16 (CNN Theory) from the Udemy course : TensorFlow 2.0: A Complete Guide on the Brand New Tensorflow

**Aprox Time:** ***3 hours***

## Day 66: 13 July 2019

- Read few TowardsDataScience articles about the understanding behind the use of a Convolutional Neural Network.

**Aprox Time:** ***1 hour***

## Day 65: 12 July 2019

- Read few articles about the understanding behind Deep Learning and the use of an Artificial Neural Network on different cases.

**Aprox Time:** ***1 hour***

## Day 64: 11 July 2019

- Done Section 3 from the Udemy course : TensorFlow 2.0: A Complete Guide on the Brand New Tensorflow

**Aprox Time:** ***1 hour***

----------------------------------------------
     Week 9 Recap - Columbia University course & new Udemy tutorial

- Done the Machine Learning for Data Science and Analytics (Columbia University) course

- Started the Udemy course : TensorFlow 2.0: A Complete Guide on the Brand New Tensorflow

----------------------------------------------

## Day 63: 10 July 2019

- Done a TensorFlow tutorial on DataCamp website.

- Done Section 2 from the Udemy course : TensorFlow 2.0: A Complete Guide on the Brand New Tensorflow

**Aprox Time:** ***1 hour***

## Day 62: 09 July 2019

- Started a TensorFlow tutorial on DataCamp website.

- Done Section 1 from the Udemy course : TensorFlow 2.0: A Complete Guide on the Brand New Tensorflow

**Aprox Time:** ***1 hour***

## Day 61: 08 July 2019

- Continued the Jupyter Notebook resuming the Mastering Machine Learning with Python in Six Steps book.

**Aprox Time:** ***1 hour***

## Day 60: 07 July 2019

- Finished the Machine Learning for Data Science and Analytics (Columbia University) course

**Aprox Time:** ***2 hours***

## Day 59: 06 July 2019

- Done Algorithms 3 and started Machine Learning sections from the Machine Learning for Data Science and Analytics (Columbia University) course

**Aprox Time:** ***1 hour***

## Day 58: 05 July 2019

- Done Algorithms 2 section from the Machine Learning for Data Science ans Analytics (Columbia University) course

**Aprox Time:** ***1 hour***

## Day 57: 04 July 2019

- Done Algorithms 1 section from the Machine Learning for Data Science ans Analytics (Columbia University) course

**Aprox Time:** ***1 hour***

----------------------------------------------
     Week 8 Recap - Paste what I read on a Jupyter Notebook

- Done the Data Science Primer mini-course

- Started a Jupyter Notebook resuming the Mastering Machine Learning with Python in Six Steps book

----------------------------------------------

## Day 56: 03 July 2019

- Continued the Jupyter Notebook resuming the Mastering Machine Learning with Python in Six Steps book.

**Aprox Time:** ***1 hour***

## Day 55: 02 July 2019

- Continued the Jupyter Notebook resuming the Mastering Machine Learning with Python in Six Steps book.

- Continued the Titanic: Machine Learning from Disaster, Kaggle competition

**Aprox Time:** ***2 hours***

## Day 54: 01 July 2019

- Continued the Jupyter Notebook resuming the Mastering Machine Learning with Python in Six Steps book.

- Continued to watch Hugo Larochelle youtube playlist about Machine Learning.

**Aprox Time:** ***2 hours***

## Day 53: 30 June 2019

- Finished the Data Science Primer mini-course.

- Continued the Titanic: Machine Learning from Disaster, Kaggle competition

**Aprox Time:** ***2 hours***

## Day 52: 29 June 2019

- Continued the Titanic: Machine Learning from Disaster, Kaggle competition

**Aprox Time:** ***1 hour***

## Day 51: 28 June 2019

- Continued the Data Science Primer mini-course.

- Done some IA exercices on Hacker Rank

**Aprox Time:** ***1 hour***

## Day 50: 27 June 2019

- Continued the Jupyter Notebook resuming the Mastering Machine Learning with Python in Six Steps book.

- Started to watch Hugo Larochelle youtube playlist about Machine Learning.

**Aprox Time:** ***1 hour***

----------------------------------------------
     Week 7 Recap - Discovering Jupyter Notebook

- Discovered the use of a Jupyter Notebook and started writing some  

- Started to read the Mastering Machine Learning with Python in Six Steps book.

----------------------------------------------

## Day 49: 26 June 2019

- Continued the Jupyter Notebook resuming the Mastering Machine Learning with Python in Six Steps book.

- Continued the Data Science Primer mini-course.

**Aprox Time:** ***1 hour***

## Day 48: 25 June 2019

- Started a Jupyter Notebook resuming the Mastering Machine Learning with Python in Six Steps book.

- Continued the Data Science Primer mini-course.

**Aprox Time:** ***1 hour***

## Day 47: 24 June 2019

- Followed a guide on how to write on Jupyter Notebook

- Done a Jupyter Notebook describing the process to analyze data with the Iris flower data set.

- Started the Data Science Primer mini-course.

**Aprox Time:** ***2 hours***

## Day 46: 23 June 2019

- Continued to read the Mastering Machine Learning with Python in Six Steps book.

**Aprox Time:** ***1 hour***

## Day 45: 22 June 2019

- Continued to read the Mastering Machine Learning with Python in Six Steps book.

**Aprox Time:** ***1 hour***

## Day 44: 21 June 2019

- Continued to read the Mastering Machine Learning with Python in Six Steps book.

- Started the Titanic: Machine Learning from Disaster, Kaggle competition

**Aprox Time:** ***2 hours***

## Day 43: 20 June 2019

- Continued to read the Mastering Machine Learning with Python in Six Steps book.

**Aprox Time:** ***1 hour***

----------------------------------------------
     Week 6 Recap - My first ANN & CNN

- Read some articles talking about Natural Language Pre-processing and Deep Learning.  

- Read the book : Learn Machine Learning In One Day.  

- Done a ANN and CNN with the help of the Hadelin de Ponteves tutorial.  

----------------------------------------------

## Day 42: 19 June 2019

- Finished to code an Convolutional Neural Network following Hadelin de Ponteves tutorial.

- Done the Kaggle tutorial : Intro to Machine Learning.

- Started to read the Mastering Machine Learning with Python in Six Steps book.

**Aprox Time:** ***5 hours***

## Day 41: 18 June 2019

- Finished to code an Artificial Neural Network following Hadelin de Ponteves tutorial

**Aprox Time:** ***1 hour***

## Day 40: 17 June 2019

- Started to code an Artificial Neural Network following Hadelin de Ponteves tutorial

- Followed course that introduce pandas library.

**Aprox Time:** ***2 hours***

## Day 39: 16 June 2019

- Read the AIsciences bonus chapter about the Introduction to Neural Network and Deep Learning

- Read some TowardsDataScience articles about Natural Language Pre-processing method.

**Aprox Time:** ***3.5 hours***

## Day 38: 15 June 2019

- Had some fun analyzing and using different Supervised and Unsupervised algorithms on the Iris data set.

- Started to develop an algorithm that will analyze the sentiment behind an Hashtag.

**Aprox Time:** ***4 hours***

## Day 37: 14 June 2019

- Finished to read the book : Learn Machine Learning In One Day

**Aprox Time:** ***2 hours***

## Day 36: 13 June 2019

- Started to read the book : Learn Machine Learning In One Day

**Aprox Time:** ***2 hours***

----------------------------------------------
     Week 5 Recap - Lecture Time ! (again)

- Finished to read the book : Python for Data Analysis.

----------------------------------------------

## Day 35: 12 June 2019

- Assisted to a class about an Introduction on python.

**Aprox Time:** ***2 hours***

## Day 34: 11 June 2019

- Finally finished the book : Python for Data Analysis.

**Aprox Time:** ***1 hour***

## Day 33: 10 June 2019

- Read chapter ten and eleven of the book Python for Data Analysis.

**Aprox Time:** ***1.5 hour***

## Day 32: 09 June 2019

- Read chapter nine of the book Python for Data Analysis.

- Revised what I learned about Natural Language Processing.

**Aprox Time:** ***2 hours***

## Day 31: 08 June 2019

- Read chapter eight of the book Python for Data Analysis.

**Aprox Time:** ***1 hour***

## Day 30: 07 June 2019

- Read chapter six and seven of the book Python for Data Analysis.

- Finished to watch the video : Learn Machine Learning: Crash Course for Beginners from freeCodeCamp channel

**Aprox Time:** ***1.5 hour***

## Day 29: 06 June 2019

- Read chapter five of the book Python for Data Analysis.

- Started to watch the video : Learn Machine Learning: Crash Course for Beginners from freeCodeCamp channel

**Aprox Time:** ***1 hour***

----------------------------------------------
     Week 4 Recap - Video & Lecture Time ! 

- Watched a lot of video talking about how Deep Learning works

- Started to read the book : Python for Data Analysis.

----------------------------------------------

## Day 28: 05 June 2019

- Read chapter four of the book Python for Data Analysis.

**Aprox Time:** ***1 hour***

## Day 27: 04 June 2019

- Read chapter three of the book Python for Data Analysis.

- Started to learn things about the CNN method.

**Aprox Time:** ***1 hour***

## Day 26: 03 June 2019

- Read chapter one and two of the book Python for Data Analysis.

- Watched The wonderful and terrifying implications of computers that can learn from TED Talks.

**Aprox Time:** ***2 hours***

## Day 25: 02 June 2019

- Learned how to manipulate data in Python.

**Aprox Time:** ***1 hour***

## Day 24: 01 June 2019

- Watched Le Deep Learning video from ScienceEtonnante's youtube channel

- Watched How we're teaching computers to understand pictures from TED Talks

- Watched L'apprentissage profond: une révolution en intelligence artificielle video from Yann LeCun

**Aprox Time:** ***2 hours***

## Day 23: 31 May 2019

- Learned about Linear Regression with Multiple Variable.

**Aprox Time:** ***1 hour***

## Day 22: 30 May 2019

- Done the HMMs and Filters section from the Intro to AI Udacity course.

**Aprox Time:** ***1 hour***

----------------------------------------------
     Week 3 Recap - Revisions and Natural Language Processing

- Revised everything that I learned till now

- Continued the Intro to AI Udacity course

----------------------------------------------

## Day 21: 29 May 2019

- Done the Reinforcement Learning section from the Intro to AI Udacity course.

**Aprox Time:** ***1 hour***

## Day 20: 28 May 2019

- Done the Natural Language Processing II section from the Intro to AI Udacity course.

**Aprox Time:** ***1 hour***

## Day 19: 27 May 2019

- Done the Natural Language Processing section from the Intro to AI Udacity course.

**Aprox Time:** ***1.5 hour***

## Day 18: 26 May 2019

- Revised Claffisication section.

**Aprox Time:** ***1 hour***

## Day 17: 25 May 2019

Phew! had midterms today but still managed to code a little 😅
I think I'll be more able to code now on. 😵

- Done Linear Algebra online lessons.

**Aprox Time:** ***1 hour***

## Day 15: 24 May 2019

- Continued to revise everything that I learned and saw for the past 2 weeks.

- Listened to The "Dashboard Conspiracy" with Lyn Bartram and Alper Sarikaya podcast from the Data Stories channel.

**Aprox Time:** ***1.5 hour***

## Day 16: 23 May 2019

- Done a  Linear Regression with One Variable online lesson.

**Aprox Time:** ***1.5 hour***

## Day 15: 22 May 2019

- Continued to revise everything that I learned and saw for the past 2 weeks.

- Listened to The "Dashboard Conspiracy" with Lyn Bartram and Alper Sarikaya podcast from the Data Stories channel.

**Aprox Time:** ***1.5 hour***

----------------------------------------------
     Week 2 Recap - Classification method and Unsupervised Learning

- Learned around 6 Classification algorithms and how to implement them mainly in Python.
![Classification Algorithms 1](/Calendar/img/classification1.png)
![Classification Algorithms 2](/Calendar/img/classification2.png)

- Can now increase the dimension of a 2D space in case to make a linear separation.

- Learned around 2 Clustering algorithms  and how to implement them mainly in Python.
![Clustering Algorithms](/Calendar/img/clustering.png)
- Learned the Apriori and Eclat algorithms

- Learned the UCB and Thompson algorithms
![Reinforcement Learning](/Calendar/img/reinforcement.png)
- Learned about the Natural Language Processing method

----------------------------------------------

## Day 14: 21 May 2019

For the next few days I will devote a minimum of time to code because of personal reasons :/  

- Started to revise everything that I learned and saw for the past 2 weeks.

**Aprox Time:** ***1 hour***

## Day 13: 20 May 2019

- Done the Reinforcement Learning and Natural Language Processing sections from the Machine Learning: A-Z course.

**Aprox Time:** ***4 hours***

## Day 12: 19 May 2019

- Done the Clustering and Association Rule sections from the Machine Learning: A-Z course.

- Read some documentation about the Apriori and Eclat algorithms.

**Aprox Time:** ***5 hours***

## Day 11: 18 May 2019

- Visualised Decision Tree and Random Forest Classification algorithms in Python.

- Learned how to tell an approximate accuracy ratio with the CAP method.

**Aprox Time:** ***3 hours***

## Day 10: 17 May 2019

Went to relax on musical swings between two coding sessions today 😄

- Learned how to implement the SVM algorithm in Python.

- Learned how to linearly separate data points with the kernel trick.

**Aprox Time:** ***1.5 hour***

## Day 09: 16 May 2019

- Started to learn about Classification method with the Logistic Regression algorithm.

- Listened the Data Visualization at Twitter podcast from the Data Stories channel.

**Aprox Time:** ***1.5 hour***

## Day 08: 15 May 2019

- Read the Supervised Learning condensed lesson from the Stanford CS 229 class

**Aprox Time:** ***1 hour***

----------------------------------------------
     Week 1 Recap - Regression Method

- Learned around 8 Regression algorithms and how to implement them mainly in Python with the breslow dataset.
![Regression Algorithms 1](/Calendar/img/regression1.png)
![Regression Algorithms 2](/Calendar/img/regression2.png)

- Can now calculate the Least Square Regression

- Read the Second Chapter from the Introduction to Machine Learning with Python book

- Listened to 3 DataSkeptic podcasts

----------------------------------------------

## Day 07: 14 May 2019

- Learned about the Least Square Regression method and done some exercises

**Aprox Time:** ***1.5 hour***

## Day 06: 13 May 2019

- Learned to implement Decision Tree Regression and Random Forest Regression algorithms in Python

**Aprox Time:** ***3 hours***

## Day 05: 12 May 2019

- Listened to the Attention podcast from DataSkeptic show
- Learned to implement Polynomial Regression and SVR algorithms in Python

**Aprox Time:** ***2 hours***

## Day 04: 11 May 2019

A little day today, I had water damage in my apartment 😬🔫

- Started to read the Unsupervised Learning and Preprocessing chapter from the book: Introduction to Machine Learning with Python

**Aprox Time:** ***1 hour***

## Day 03: 10 May 2019

Enjoyed this sunny day with some code and tea! 🌞

- Listened Sentiment Analysis and Human vs Machine Transcription podcasts from DataSkeptic
- Read Supervised Learning chapter from the book: Introduction to Machine Learning with Python

**Aprox Time:** ***2 hours***

## Day 02: 09 May 2019

Enjoyed this sunny day with some code and tea! 🌞

- Learned to implement Multiple Linear Regression algorithm in Python  
- Learned to implement k-nearest neighbors Classification and Regression algorithms in Python

**Aprox Time:** ***4 hours***

## Day 01: 08 May 2019

### Hello World!

Starting a second round of this challenge, specialised in Machine Learning 😎

- Learned about the Simple Linear Regression and how to implement it in Python and R  
- Read documentation about Classification and Regression methods

**Aprox Time:** ***2.5 hours***
